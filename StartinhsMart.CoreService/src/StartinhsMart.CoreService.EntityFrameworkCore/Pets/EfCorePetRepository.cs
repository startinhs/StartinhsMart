    ...
    public class EfCorePetRepository : EfCoreRepository<CoreServiceDbContext, Pet, Guid>, IPetRepository
    {
            ...
            bool? isStock = null)
        {
            return query
                    ...
                    .WhereIf(!string.IsNullOrWhiteSpace(gender), e => e.Gender != null && e.Gender.Contains(gender!))
                    .WhereIf(!string.IsNullOrWhiteSpace(color), e => e.Color != null && e.Color.Contains(color!))
                    .WhereIf(weightMin.HasValue, e => e.Weight >= weightMin!.Value)
                    .WhereIf(weightMax.HasValue, e => e.Weight <= weightMax!.Value)
                    .WhereIf(!string.IsNullOrWhiteSpace(healthStatus), e => e.HealthStatus != null && e.HealthStatus.Contains(healthStatus!))
